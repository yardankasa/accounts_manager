...
async def add_account_as_channel_admin_on_node(
    node: dict,
    session_path: str,
    api_id: int,
    api_hash: str,
    ...
    """
    Add the account (session) as admin in the channel. Returns (success, error_message).
    Main node: runs locally. Remote: runs add_admin_worker via SSH.
    """
    if node.get("is_main"):
        from . import session_status
        return await session_status.add_account_as_channel_admin(
            session_path, api_id, api_hash, channel_id
        )
    # Remote: SSH and run add_admin_worker
    host = node.get("ssh_host")
    user = node.get("ssh_user")
    port = int(node.get("ssh_port") or 22)
    kwargs = {"host": host, "port": port, "username": user}
    ...
    worker_path = _APP_ROOT / "scripts" / "add_admin_worker.py"
    if not worker_path.exists():
        return False, "Add admin worker not found"
    script_content = worker_path.read_text()
    remote_path = f"/tmp/rezabots_add_admin_{node.get('id', 0)}.py"
    try:
        conn = await asyncio.wait_for(asyncssh.connect(**kwargs), timeout=15)
        try:
            async with conn.start_sftp_client() as sftp:
        ...
        logger.exception("Add admin on node failed: %s", e)
